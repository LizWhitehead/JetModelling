    ...
    try:
        ...
        ridge2, phi_val2, Rlen2, Error = FindRidges(area_fluxes, init_point, R, dphi, lmsize)
            ...
            if np.all(np.isnan(ridge1)) and np.all(np.isnan(ridge2)):
                ...
                np.savetxt(RLF.R1 %(source_name, str(JMS.map_number+1)), file1, delimiter=' ', \
                           header='ridgepoint x-coord (pix), ridgepoint y-coord (pix), ridgepoint phi (radns), ridgepoint R (pix)')
                np.savetxt(RLF.R2 %(source_name, str(JMS.map_number+1)), file2, delimiter=' ', \
                           header='ridgepoint x-coord (pix), ridgepoint y-coord (pix), ridgepoint phi (radns), ridgepoint R (pix)')
                ...
                y_plotlimits = np.ma.masked_array(y, mask=np.ma.masked_where(y < (JMC.nSig[0] * JMS.bgRMS), y, copy=True).mask)
                x_plotlimits = np.ma.masked_array(x, np.ma.masked_where(x < (JMC.nSig[0] * JMS.bgRMS), x, copy=True).mask)
                ...
                fig.savefig(RLF.Rimage %(source_name, str(JMS.map_number+1), dphi))
                plt.close(fig)
